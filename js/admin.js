document.addEventListener('DOMContentLoaded', () => {
  ...
  function renderToursTable() {
    ...
    toursData.forEach(t => {
      ...
      tr.innerHTML = `
        ...
        <td>${t.title}</td>
        <td>
          <!-- Чекбокс-переключатель активности -->
  ...
  toursTableBody.addEventListener('click', (e) => {
    if (e.target.tagName === 'BUTTON') {
      ...
      if (action === 'edit') {
        ...
        if (found) {
          ...
          tourPriceGroup.value = found.priceGroup || 0;
          tourPriceIndividual.value = found.priceIndividual || 0;
        }
    ...
    // Если клик по чекбоксу toggleActive
    if (e.target.type === 'checkbox' && e.target.getAttribute('data-action') === 'toggleActive') {
      const id = e.target.getAttribute('data-id');
      const found = toursData.find(x => x.id === id);
      if (found) {
        found.active = e.target.checked;
        saveToursToStorage(toursData);
        // Можно перерендерить, чтобы порядок (активные сверху) отразился в фронте, 
        // но здесь только в админке — на сайте уже пересортирует script.js.
        renderToursTable();
      }
    }
  });
  ...
  saveTourBtn.addEventListener('click', () => {
    ...
    if (!found) {
      ...
      const newTour = {
        ...
        priceGroup: parseInt(tourPriceGroup.value, 10) || 0,
        priceIndividual: parseInt(tourPriceIndividual.value, 10) || 0
      };
      ...
      found.priceGroup = parseInt(tourPriceGroup.value, 10) || 0;
      found.priceIndividual = parseInt(tourPriceIndividual.value, 10) || 0;
    }
  ...
  reviewResetFormBtn.addEventListener('click', resetReviewForm);

  saveReviewBtn.addEventListener('click', () => {
    const idx = editReviewIndex.value.trim();
    if (!reviewAuthor.value.trim() || !reviewText.value.trim()) {
      alert('Введите автора и текст отзыва!');
      return;
    }
    if (idx === "") {
      // Новый отзыв
      siteContent.reviews.push({
        ...
        social: reviewSocial.value.trim()
      });
    } else {
      // Редактируем
      siteContent.reviews[idx].author = reviewAuthor.value.trim();
      siteContent.reviews[idx].text = reviewText.value.trim();
      siteContent.reviews[idx].social = reviewSocial.value.trim();
    }
    saveSiteContent(siteContent);
    renderReviewsTable();
    resetReviewForm();
